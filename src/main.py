...
class GeneticAlgorithm:
    ...
    def setup_components(self):
        self.creator = SDXLPromptEmbeddingImageCreator(batch_size=1, inference_steps=self.config['inference_steps'], deterministic=self.config['deterministic'])
        ...
        self.mutator = PooledUniformGaussianMutator(
            UniformGaussianMutatorArguments(
                ...
                mutation_strength=2,
                clamp_range=(SDXLTurboEmbeddingRange().minimum, SDXLTurboEmbeddingRange().maximum),
            ),
            UniformGaussianMutatorArguments(
                mutation_rate=self.config['mutation_rate'],
                mutation_strength=0.4,
                clamp_range=(SDXLTurboPooledEmbeddingRange().minimum, SDXLTurboPooledEmbeddingRange().maximum),
            )
        )
    ...
    def run(self):
        ...
        folder_path = os.path.join("saved_images", f"{current_timestamp}_{formatted_prompt}")

        population = [SolutionCandidate(self.creator.arguments_from_prompt(prompt), None) for _ in range(self.config['pop_size'])]
        stored_elites = []
        ...
        try:
            for generation in range(self.config['num_generations']):
                ...
                stored_elites = elite_individuals if elite_individuals else stored_elites
                new_population = stored_elites.copy()

                mating_pool = [self.selector.select([x[1] for x in fitness_scores]) for _ in range(self.config['pop_size'])]
                while len(new_population) < self.config['pop_size']:
                    parent_indices = np.random.choice(len(mating_pool), 2, replace=False)
                    parent1, parent2 = (mating_pool[parent_indices[0]], mating_pool[parent_indices[1]])
                    child = self.crossover.crossover(parent1.arguments, parent2.arguments) if np.random.rand() < self.config['crossover_rate'] else parent1.arguments
                    new_population.append(SolutionCandidate(self.mutator.mutate(child), None))
