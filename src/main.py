...
creator = SDXLPromptEmbeddingImageCreator(batch_size=1, inference_steps=config['inference_steps'], deterministic=config['deterministic'])
crossover = PooledArithmeticCrossover(
    interpolation_weight=0.5,
    interpolation_weight_pooled=0.5,
    proportion=config['crossover_proportion'],
    proportion_pooled=config['crossover_proportion'],
)
...
population = [SolutionCandidate(initial_argument, None) for _ in range(config['pop_size'])]
...
try:
    for generation in range(config['num_generations']):
        ...
        mating_pool = [
            selector.select([x[1] for x in fitness_scores]) for _ in range(config['pop_size'])
        ]
